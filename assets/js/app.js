...
const quickCopySettingsToggle = document.getElementById("quick-copy-settings-toggle");
const quickCopySettingsPanel = document.getElementById("quick-copy-settings-panel");
const quickCopyActions = document.getElementById("quick-copy-actions");
const quickCopyEmpty = document.getElementById("quick-copy-empty");
const quickCopyNote = document.getElementById("quick-copy-note");
const quickCopyLibrary = document.getElementById("quick-copy-library");
const quickCopyName = document.getElementById("quick-copy-name");
const quickCopyText = document.getElementById("quick-copy-text");
const quickCopySave = document.getElementById("quick-copy-save");
const quickCopyClear = document.getElementById("quick-copy-clear");
...
const QUICK_COPY_STORAGE_KEY = "kamili-quick-copy-items-v1";
...
body[data-theme="light"] .workflow-embedded-prompt-shell {
  background:
    linear-gradient(180deg, rgba(248, 250, 253, 0.95), rgba(238, 243, 248, 0.98)),
...
body[data-theme="light"] .workflow-embedded-prompt-shell .prompt-banner-params-inline .prompt-param {
  border-color: rgba(15, 23, 42, 0.12);
  background:
    linear-gradient(180deg, rgba(249, 251, 254, 0.96), rgba(239, 244, 249, 0.96)),
...
body[data-theme="light"] .workflow-embedded-prompt-shell .language-dropdown {
  background:
    linear-gradient(180deg, rgba(249, 251, 254, 0.98), rgba(239, 244, 249, 0.98)),
    rgba(247, 250, 252, 0.96);
  box-shadow: 0 18px 34px rgba(15, 23, 42, 0.12);
}

body[data-theme="light"] .workflow-embedded-prompt-shell .language-option:hover,
body[data-theme="light"] .workflow-embedded-prompt-shell .language-option.is-active {
  color: var(--text);
}
...
body[data-theme="light"] .workflow-embedded-prompt-shell .language-custom {
  border-top-color: rgba(15, 23, 42, 0.08);
}

body[data-theme="light"] .workflow-embedded-prompt-shell .segmented-control {
  background: linear-gradient(180deg, rgba(243, 247, 251, 0.92), rgba(233, 239, 245, 0.96));
  box-shadow:
    inset 0 1px 0 rgba(255, 255, 255, 0.88),
    0 10px 20px rgba(15, 23, 42, 0.04);
}

body[data-theme="light"] .workflow-embedded-prompt-shell .segmented-tab {
  color: #516072;
}

body[data-theme="light"] .workflow-embedded-prompt-shell .segmented-tab:hover {
  color: #1f2f42;
  background: rgba(15, 23, 42, 0.055);
}

body[data-theme="light"] .workflow-embedded-prompt-shell .segmented-tab.is-locked {
  color: #7b8796;
  opacity: 1;
}

body[data-theme="light"] .workflow-embedded-prompt-shell .segmented-icon-button {
  background: linear-gradient(180deg, rgba(249, 251, 254, 0.96), rgba(239, 244, 249, 0.96));
  box-shadow:
    inset 0 1px 0 rgba(255, 255, 255, 0.9),
    0 10px 20px rgba(15, 23, 42, 0.05);
}

body[data-theme="light"] .workflow-embedded-prompt-shell .tabs-hint,
body[data-theme="light"] .workflow-embedded-prompt-shell .label {
  color: #526174;
}

body[data-theme="light"] .workflow-embedded-prompt-shell .prompt-builder-reference,
body[data-theme="light"] .workflow-embedded-prompt-shell .output-editor {
  border-color: rgba(15, 23, 42, 0.12);
  background: linear-gradient(180deg, rgba(250, 252, 255, 0.98), rgba(241, 245, 250, 0.99));
  box-shadow:
    inset 0 1px 0 rgba(255, 255, 255, 0.92),
    0 14px 28px rgba(15, 23, 42, 0.05);
}

body[data-theme="light"] .workflow-embedded-prompt-shell .output-editor:focus,
body[data-theme="light"] .workflow-embedded-prompt-shell .prompt-builder-reference:focus {
  border-color: rgba(82, 184, 255, 0.3);
  background: linear-gradient(180deg, rgba(252, 253, 255, 1), rgba(239, 244, 250, 1));
}

body[data-theme="light"] .workflow-embedded-prompt-shell textarea::placeholder {
  color: rgba(82, 97, 117, 0.72);
}
...
let quickCopyItems = [];
let editingQuickCopyId = "";
let copiedQuickCopyId = "";
let quickCopyCopiedPhase = "";
let quickCopyCopiedFadeTimer = 0;
let quickCopyCopiedResetTimer = 0;
...
function createDefaultQuickCopyItems() {
  return [
    {
      id: "quick-theme-lock",
      name: "Theme Lock",
      text: "Keep the same visual theme, palette, mood, and material treatment across the full set."
    },
    {
      id: "quick-no-text",
      name: "Instru 1",
      text: "No readable text, no logos, no watermarks, no UI elements, and no captions."
    },
    {
      id: "quick-doc-tone",
      name: "Instru 2",
      text: "Modern cinematic documentary mood, grounded realism, restrained contrast, clean framing, and subtle atmosphere."
    }
  ];
}

function getQuickCopyDefaultName(id) {
  switch (id) {
    case "quick-theme-lock":
      return "Theme Lock";
    case "quick-no-text":
      return "Instru 1";
    case "quick-doc-tone":
...
function migrateQuickCopyName(id, name) {
  const normalized = normalizeInlineText(name || "");
  if (!normalized) {
    return getQuickCopyDefaultName(id) || "Untitled";
  }

  if (id === "quick-no-text" && normalized === "No Text") {
    return "Instru 1";
  }

  if (id === "quick-doc-tone" && normalized === "Doc Tone") {
    return "Instru 2";
  }
...
function normalizeQuickCopyText(text) {
  return String(text || "").replace(/\r\n/g, "\n").trim();
}

function ensureQuickCopyItemsShape(items, { useDefaults = true } = {}) {
  if (!Array.isArray(items)) {
    return useDefaults ? createDefaultQuickCopyItems() : [];
  }

  const normalized = items
    .map(item => ({
      id: typeof item.id === "string" && item.id ? item.id : `quick-copy-${Date.now()}-${Math.random().toString(16).slice(2, 8)}`,
      name: migrateQuickCopyName(
        typeof item.id === "string" && item.id ? item.id : "",
        item.name || getQuickCopyDefaultName(item.id) || "Untitled"
      ),
      text: normalizeQuickCopyText(item.text || "")
    }))
...
function saveQuickCopyItems() {
  localStorage.setItem(QUICK_COPY_STORAGE_KEY, JSON.stringify(quickCopyItems));
}

function getQuickCopyPreviewText(text) {
  const compact = normalizeInlineText(text);
  if (compact.length <= 90) return compact;
  return `${compact.slice(0, 87)}...`;
}

function setQuickCopyNote(message, kind = "") {
  if (!quickCopyNote) return;
  quickCopyNote.textContent = message;
  quickCopyNote.classList.remove("is-success", "is-error");
  if (kind === "success" || kind === "error") {
    quickCopyNote.classList.add(`is-${kind}`);
  }
}

function updateQuickCopySaveLabel() {
  if (!quickCopySave) return;
  const label = quickCopySave.querySelector(".glow-button__content span");
  if (!label) return;
  label.textContent = editingQuickCopyId ? "Update Instruction" : "Save Instruction";
}

function resetQuickCopyForm() {
  editingQuickCopyId = "";
  if (quickCopyName) quickCopyName.value = "";
  if (quickCopyText) quickCopyText.value = "";
  updateQuickCopySaveLabel();
  renderQuickCopyLibrary();
}

function fillQuickCopyForm(item) {
  if (!item) {
    resetQuickCopyForm();
    return;
  }
  ...
  editingQuickCopyId = item.id;
  if (quickCopyName) quickCopyName.value = item.name;
  if (quickCopyText) quickCopyText.value = item.text;
  updateQuickCopySaveLabel();
  renderQuickCopyLibrary();
...
function renderQuickCopyButtons() {
  if (!quickCopyActions || !quickCopyEmpty) return;

  quickCopyActions.innerHTML = "";
  const hasItems = quickCopyItems.length > 0;
  quickCopyEmpty.hidden = hasItems;

  quickCopyItems.forEach(item => {
    const button = document.createElement("button");
    button.type = "button";
    button.className = "quick-copy-chip";
    button.dataset.quickCopyId = item.id;
    if (item.id === copiedQuickCopyId && quickCopyCopiedPhase === "copied") {
      button.classList.add("is-copied");
    }
    if (item.id === copiedQuickCopyId && quickCopyCopiedPhase === "fading") {
      button.classList.add("is-fading");
    }
    button.innerHTML = `<span>${escapeHtml(item.name)}</span>`;
    quickCopyActions.appendChild(button);
  });
}

function clearQuickCopyCopiedTimers() {
  if (quickCopyCopiedFadeTimer) {
    window.clearTimeout(quickCopyCopiedFadeTimer);
    quickCopyCopiedFadeTimer = 0;
  }
  if (quickCopyCopiedResetTimer) {
    window.clearTimeout(quickCopyCopiedResetTimer);
    quickCopyCopiedResetTimer = 0;
  }
}

function syncQuickCopyCopiedClasses() {
  if (!quickCopyActions) return;

  quickCopyActions.querySelectorAll(".quick-copy-chip").forEach(button => {
    const isTarget = button.dataset.quickCopyId === copiedQuickCopyId;
    button.classList.toggle("is-copied", isTarget && quickCopyCopiedPhase === "copied");
    button.classList.toggle("is-fading", isTarget && quickCopyCopiedPhase === "fading");
  });
}

function resetQuickCopyCopiedState() {
  clearQuickCopyCopiedTimers();
  copiedQuickCopyId = "";
  quickCopyCopiedPhase = "";
  syncQuickCopyCopiedClasses();
}

function renderQuickCopyLibrary() {
  if (!quickCopyLibrary) return;

  quickCopyLibrary.innerHTML = "";

  if (!quickCopyItems.length) {
    const empty = document.createElement("div");
    empty.className = "quick-copy-library-empty";
    empty.textContent = "No instructions yet. Add one below.";
    quickCopyLibrary.appendChild(empty);
    return;
  }

  quickCopyItems.forEach(item => {
    const card = document.createElement("div");
    card.className = "quick-copy-item";
    if (item.id === editingQuickCopyId) {
      card.classList.add("is-active");
    }

    const editButton = document.createElement("button");
    editButton.type = "button";
    editButton.className = "quick-copy-item-main";
    editButton.dataset.quickCopyEdit = item.id;
    editButton.innerHTML = `<strong>${escapeHtml(item.name)}</strong><span>${escapeHtml(getQuickCopyPreviewText(item.text))}</span>`;

    const meta = document.createElement("button");
    meta.type = "button";
    meta.className = "quick-copy-item-meta";
    meta.dataset.quickCopyEdit = item.id;
    meta.textContent = item.id === editingQuickCopyId ? "Editing" : "Edit";

    const deleteButton = document.createElement("button");
    deleteButton.type = "button";
    deleteButton.className = "icon-button quick-copy-item-delete";
    deleteButton.dataset.quickCopyDelete = item.id;
    ...
    quickCopyLibrary.appendChild(card);
  });
}

function renderQuickCopySection() {
  renderQuickCopyButtons();
  renderQuickCopyLibrary();
  updateQuickCopySaveLabel();
}

function openQuickCopySettings() {
  if (!quickCopySettingsPanel || !quickCopySettingsToggle) return;
  quickCopySettingsPanel.hidden = false;
  quickCopySettingsToggle.classList.add("is-active");
  quickCopySettingsToggle.setAttribute("aria-expanded", "true");
}

function closeQuickCopySettings() {
  if (!quickCopySettingsPanel || !quickCopySettingsToggle) return;
  quickCopySettingsPanel.hidden = true;
  quickCopySettingsToggle.classList.remove("is-active");
  quickCopySettingsToggle.setAttribute("aria-expanded", "false");
}

function toggleQuickCopySettings() {
  if (!quickCopySettingsPanel) return;
  if (quickCopySettingsPanel.hidden) {
    openQuickCopySettings();
    return;
  }
  closeQuickCopySettings();
}

function loadQuickCopyItems() {
  try {
    const raw = localStorage.getItem(QUICK_COPY_STORAGE_KEY);
    if (raw === null) {
      quickCopyItems = createDefaultQuickCopyItems();
      saveQuickCopyItems();
    } else {
      quickCopyItems = ensureQuickCopyItemsShape(JSON.parse(raw), { useDefaults: false });
    }
  } catch (error) {
    quickCopyItems = createDefaultQuickCopyItems();
    saveQuickCopyItems();
  ...
  renderQuickCopySection();
  setQuickCopyNote(quickCopyItems.length ? "Tap a button to copy its instruction." : "Add your first instruction from settings.");
}

async function copyQuickCopyItem(id) {
  const item = quickCopyItems.find(entry => entry.id === id);
  if (!item) return;

  try {
    await navigator.clipboard.writeText(item.text);
    clearQuickCopyCopiedTimers();
    copiedQuickCopyId = item.id;
    quickCopyCopiedPhase = "copied";
    syncQuickCopyCopiedClasses();
    setQuickCopyNote(`${item.name} copied.`, "success");

    quickCopyCopiedFadeTimer = window.setTimeout(() => {
      if (copiedQuickCopyId !== item.id) return;
      quickCopyCopiedPhase = "fading";
      syncQuickCopyCopiedClasses();
    }, 3000);

    quickCopyCopiedResetTimer = window.setTimeout(() => {
      if (copiedQuickCopyId !== item.id) return;
      resetQuickCopyCopiedState();
    }, 4300);
  } catch (error) {
    setQuickCopyNote("Copy failed.", "error");
  }
}

function saveQuickCopyItemFromForm() {
  if (!quickCopyName || !quickCopyText) return;

  const name = normalizeInlineText(quickCopyName.value);
  const text = normalizeQuickCopyText(quickCopyText.value);

  if (!name || !text) {
    setQuickCopyNote("Add both a button name and instruction text.", "error");
    return;
  }

  if (editingQuickCopyId) {
    quickCopyItems = quickCopyItems.map(item => item.id === editingQuickCopyId ? { ...item, name, text } : item);
  } else {
    quickCopyItems.push({
      id: `quick-copy-${Date.now()}-${Math.random().toString(16).slice(2, 8)}`,
      name,
      text
    });
  }
  ...
  resetQuickCopyCopiedState();
  renderQuickCopySection();
  setQuickCopyNote(`${name} saved.`, "success");
  resetQuickCopyForm();
}

function deleteQuickCopyItem(id) {
  const item = quickCopyItems.find(entry => entry.id === id);
  if (!item) return;

  quickCopyItems = quickCopyItems.filter(entry => entry.id !== id);
  if (editingQuickCopyId === id) {
    resetQuickCopyForm();
  }
  if (copiedQuickCopyId === id) {
    resetQuickCopyCopiedState();
  }
  saveQuickCopyItems();
  renderQuickCopySection();
  setQuickCopyNote(quickCopyItems.length ? `${item.name} deleted.` : "No instructions left. Add a new one from settings.", quickCopyItems.length ? "success" : "");
}
...
function syncWorkflowFrameTheme(theme = document.body.dataset.theme || "dark") {
  if (!workflowFrame) {
    return;
  }
  ...
  try {
    ...
    console.warn("Could not sync workflow theme:", error);
  }
}
...
if (quickCopySettingsToggle) {
  quickCopySettingsToggle.addEventListener("click", event => {
    event.stopPropagation();
    toggleQuickCopySettings();
  });
}
if (quickCopyActions) {
  quickCopyActions.addEventListener("click", event => {
    const button = event.target.closest("[data-quick-copy-id]");
    if (!button) return;
    void copyQuickCopyItem(button.dataset.quickCopyId);
  });
}
if (quickCopyLibrary) {
  quickCopyLibrary.addEventListener("click", event => {
    event.stopPropagation();

    const deleteButton = event.target.closest("[data-quick-copy-delete]");
    if (deleteButton) {
      deleteQuickCopyItem(deleteButton.dataset.quickCopyDelete);
      return;
    }

    const editButton = event.target.closest("[data-quick-copy-edit]");
    if (editButton) {
      const item = quickCopyItems.find(entry => entry.id === editButton.dataset.quickCopyEdit);
      if (item) {
        fillQuickCopyForm(item);
      }
    }
  });
}
if (quickCopySave) {
  quickCopySave.addEventListener("click", saveQuickCopyItemFromForm);
}
if (quickCopyClear) {
  quickCopyClear.addEventListener("click", () => {
    resetQuickCopyForm();
    setQuickCopyNote(quickCopyItems.length ? "Tap a button to copy its instruction." : "Add your first instruction from settings.");
  });
}
document.addEventListener("click", event => {
  ...
  const clickedInsideQuickCopySettings = (quickCopySettingsPanel && quickCopySettingsPanel.contains(event.target))
    || (quickCopySettingsToggle && quickCopySettingsToggle.contains(event.target));
  if (!clickedInsideQuickCopySettings) {
    closeQuickCopySettings();
  }
});
...
if (quickCopyText) {
  quickCopyText.addEventListener("keydown", event => {
    if ((event.metaKey || event.ctrlKey) && event.key === "Enter") {
      event.preventDefault();
      saveQuickCopyItemFromForm();
    }
  });
}
...
if (workflowFrame) {
  workflowFrame.addEventListener("load", () => {
    window.requestAnimationFrame(() => {
      ...
      window.requestAnimationFrame(() => syncWorkflowFrameTheme());
    });
  });
}
