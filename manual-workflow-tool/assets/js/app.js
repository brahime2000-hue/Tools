...
const LEGACY_EXAMPLE_VALUES = new Set([
  "1AbCdEfGhIjKlMnOpQrStUvWxYz",
  "0AExampleFolderId123456789",
  "0AExampleFolderId1234567",
...
function applyStoredTheme() {
  try {
    const savedTheme = window.localStorage.getItem("kamilys-theme");
...
function restoreState() {
  try {
    ...
    const savedHasExplicitStartRow =
      saved.settings?.startRowExplicit === true ||
      (Number.isInteger(saved.settings?.startRow) && saved.settings.startRow !== DEFAULT_START_ROW);

    state.settings = {
      sheetId: sanitizeStoredSettingValue(saved.settings?.sheetId),
      gid: sanitizeStoredSettingValue(saved.settings?.gid),
      startRow: savedStartRow,
      startRowExplicit: savedHasExplicitStartRow,
      folderId: sanitizeStoredSettingValue(saved.settings?.folderId),
      clientId: sanitizeStoredSettingValue(saved.settings?.clientId),
    };
...
function sanitizeStoredSettingValue(value) {
  const normalized = typeof value === "string" ? value.trim() : "";
  if (!normalized) return "";
  return LEGACY_EXAMPLE_VALUES.has(normalized) ? "" : normalized;
}
...
function hydrateForm() {
  ...
  elements.startRow.value = state.settings.startRowExplicit ? state.settings.startRow : "";
...
function collectSettings({ allowPartial = false, requireFolderId = true } = {}) {
  const settings = {
    ...
    startRowExplicit: elements.startRow.value.trim() !== "",
...
function updateCharacterCounts() {
  if (elements.scriptCharCount) {
    const hasScriptText = elements.scriptContent.value.trim().length > 0;
    elements.scriptCharCount.textContent = formatCharacterCount(elements.scriptContent.value);
    elements.scriptCharCount.hidden = !hasScriptText;
  }
}
